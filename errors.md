...
Note that conda dependency with pinned channels (eg. `conda-forge::openjdk`) are fine
and should be handled by the linting properly.

Each dependency can have the following lint failures and warnings:

* (Test failure) Dependency does not version number pinned, eg. `toolname=1.6.8`
...
to create the container. Such Dockerfiles can usually be very minimal, eg:
...
Note that these lines must be an exact copy of the above example, with the exception that
